...
def eater_get_photo(user_email):
    try:
        ...
        logger.info(f"Received photo_data size for user {user_email}: {len(photo_data)} bytes")
        ...
        logger.info(f"Resized photo_data size for user {user_email}: {len(resized_photo_data)} bytes")
        ...
        send_kafka_message(encode_image(filename), type_of_processing, user_email, message_id=message_id)
...
def send_kafka_message(photo_base64, type_of_processing, user_email, message_id=None, topic="eater-send-photo"):
