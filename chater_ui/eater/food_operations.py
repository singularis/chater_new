...
def modify_food_record(request, user_email):
    ...
    try:
        ...
        logger.info(f"Extracted time and percentage from Protobuf for user {user_email}: time={time}, percentage={percentage}")

        producer = create_producer()
        message_id = str(uuid.uuid4())
        message = {"key": message_id, "value": {"time": time, "user_email": user_email, "percentage": percentage}}
        produce_message(producer, topic="modify_food_record", message=message)
        logger.info(f"Sent message to Kafka for user {user_email}: {message}")

        logger.info(f"Waiting for modify confirmation for user {user_email} with message ID {message_id}")
        
        # Get response from Redis using the background consumer service
        try:
            response = get_user_message_response(message_id, user_email, timeout=30)
            if response is not None:
                logger.info(f"Retrieved modify confirmation for user {user_email}: {response}")
                
                if response.get("error"):
                    logger.error(f"Error in modify response for user {user_email}: {response.get('error')}")
                ...
                logger.warning(f"Timeout waiting for modify confirmation for user {user_email} with message ID {message_id}")
                modify_food_response.success = False
                response_data = modify_food_response.SerializeToString()
                return (
                    response_data,
            ...
            logger.error(f"Failed to get modify confirmation for user {user_email}: {e}")
            modify_food_response.success = False
            response_data = modify_food_response.SerializeToString()
            return (
                response_data,
