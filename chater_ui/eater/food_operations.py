...
def delete_food(request, user_email):
    ...
    try:
        ...
        if not proto_data:
            logger.error(f"No Protobuf data found in the request from user: {user_email}")
        ...
        while retry_count < max_retries:
            for response_message in consume_messages(consumer, expected_user_email=user_email):
                try:
                    ...
                    if value_dict.get("key") != message_id:
                        logger.info(f"Skipping message with different key for user {user_email}")
                        continue
