...
def save_feedback_data(user_email, feedback_text, feedback_time=None):
    ...
    try:
        ...
        if feedback_time:
            try:
                parsed_time = datetime.fromisoformat(feedback_time.replace('Z', '+00:00'))
            except ValueError:
                logger.warning(f"Invalid timestamp format: {feedback_time}, using current time")
    ...
    try:
        ...
        for feedback_record in feedback_records:
            result.append({
                ...
                "date": feedback_record["date"].isoformat() if feedback_record["date"] else None,
...
def get_user_statistics():
    ...
    try:
        ...
        constantly_active_7_days_list = [row[0] for row in constantly_active_7_days_emails]
        ...
        statistics = {
            ...
            "avg_dishes_per_user": float(avg_dishes_per_user) if avg_dishes_per_user else 0
        }
